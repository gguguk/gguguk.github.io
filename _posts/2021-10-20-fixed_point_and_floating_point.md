...
최근 사내에서 [CS50](https://www.boostcourse.org/cs112) 스터디를 시작했습니다. 본 포스팅은 CS50의 두번째 주제인 'C언어' 강의를 듣다가 평소 궁금했던 다룬 고정 소수점(fixed point)과 부동 소수점(floating point)에 대해 추가적으로 정리하는 목적입니다.
...
$$
123.73 = 10^2 \cdot 1 + 10^1 \cdot 2 + 10^0 \cdot 3 + 10^{-1} \cdot 7 + 10^{-2} \cdot 3
$$
...
$$
\begin{align}
13 &= 2^3 \cdot 1 + 2^2 \cdot 1 + 2^1 \cdot 0 + 2^0 \cdot 1 \\[1em]
&=1101_{(2)}
\end{align}
$$
...
$$
\begin{align}
0.75 &= 2^{-1} \cdot 1 + 2^{-2} \cdot 1 \\[1em]
&= 0.11_{(2)}
\end{align}
$$
...
$$
\begin{align}
0.3 &= 2^{-1} \cdot 0 + 2^{-2} \cdot 1 + 2^{-3} \cdot 0 + 2^{-4} \cdot 0 + 2^{-5} \cdot 1 + \cdots \\[1em]
...
고정 소수점(fixed point)은정수를 표현하는 비트와 소수를 표현하는 비트수를 미리 정하고(**고정**) 해당 비트만을 활용하여 실수를 표현합니다. 

- 첫번째 비트는 `sign`(부호)을 나타냅니다 (양수는 0, 음수는 1). 
- 이후 15개의 비트와 16비트는 각각 `integer part`(정수부)과 `fractional part`(소수부)에 할당합니다. 
- 이때 정수부와 소수부의 경계를 소수점의 위치라고 생각하고 2진수로 변환된 수를 그대로 넣으면 됩니다. 
...
$$
\begin{align}
7.625 &= 2^2 \cdot 1 + 2^1 \cdot 1 + 2^0 \cdot 1 + 2^{-1} \cdot 1 + 2^{-2} \cdot 0 + 2^{-3} \cdot 1 \\[1em]
&=111.101_{(2)}
\end{align}
$$
